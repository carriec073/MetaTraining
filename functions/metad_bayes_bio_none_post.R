...
nR_S1 = counts_post %>%
  filter(feedback == 'none', video == 'attention', task == 'bio') %>%
  select(ID, left) %>%
  group_by(ID) %>%
...
Hmeta_att_bio_none_post <- metad_group(
  list(nR_S1),
  list(nR_S2),
  niter,
  nchains,
...
mcmc_att_bio_none_post <- ggs(Hmeta_att_bio_none_post) %>%
  mutate(group = 'bio_none_att_post') %>%
  filter(Parameter == "mu_logMratio")

Hmeta_att_avg_bio_none_post = summary(Hmeta_att_bio_none_post)$statistics[, 'Mean'] %>%
  as.data.frame() %>%
  head(-3)

# Save as CSV
write.csv(mcmc_att_bio_none_post, file = here::here("outputs", "mcmc_results_bio_none_post.csv"), row.names = FALSE)

# Save summary statistics
mcmc_att_bio_none_post_summary <- summary(mcmc_att_bio_none_post)
#saveRDS(mcmc_att_bio_none_summary, file = here::here("functions", "output", "mcmc_att_bio_none_summary.rds"))

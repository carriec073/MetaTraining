...
Hmeta_att_dots_T1_pre <- metad_group(
  list(nR_S1),
  list(nR_S2),
  niter,
  nchains,
...
mcmc_att_dots_T1_pre <- ggs(Hmeta_att_dots_T1_pre) %>%
  mutate(group = 'dots_T1_att_pre') %>%
  filter(Parameter == "mu_logMratio")

Hmeta_att_avg_dots_T1_pre = summary(Hmeta_att_dots_T1_pre)$statistics[, 'Mean'] %>%
  as.data.frame() %>%
  head(-3)

# Save as CSV
write.csv(mcmc_att_dots_T1_pre, file = here::here("outputs", "mcmc_results_dots_T1_pre.csv"), row.names = FALSE)

# Save summary statistics
mcmc_att_dots_T1_pre_summary <- summary(mcmc_att_dots_T1_pre)

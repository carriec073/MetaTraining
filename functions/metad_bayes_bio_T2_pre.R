...
Hmeta_att_bio_T2_pre <- metad_group(
  list(nR_S1),
  list(nR_S2),
  niter,
  nchains,
...
Hmeta_meta_bio_T2_pre <- metad_group(
  list(nR_S1),
  list(nR_S2),
  niter,
  nchains,
...
mcmc_att_bio_T2_pre <- ggs(Hmeta_att_bio_T2_pre) %>%
  mutate(group = 'bio_T2_att_pre') %>%
  filter(Parameter == "mu_logMratio")
mcmc_meta_bio_T2_pre <- ggs(Hmeta_meta_bio_T2_pre) %>%
  mutate(group = 'bio_T2_meta_pre') %>%
  filter(Parameter == "mu_logMratio")
mcmc_bio_T2_pre = rbind(mcmc_att_bio_T2_pre, mcmc_meta_bio_T2_pre)

Hmeta_att_avg_bio_T2_pre = summary(Hmeta_att_bio_T2_pre)$statistics[, 'Mean'] %>%
  as.data.frame() %>%
  head(-3)
Hmeta_meta_avg_bio_T2_pre = summary(Hmeta_meta_bio_T2_pre)$statistics[, 'Mean'] %>%
  as.data.frame() %>%
  head(-3)

# Save as CSV
write.csv(mcmc_att_bio_T2_pre, file = here::here("outputs", "mcmc_results_att_bio_T2_pre.csv"), row.names = FALSE)
write.csv(mcmc_meta_bio_T2_pre, file = here::here("outputs", "mcmc_results_meta_bio_T2_pre.csv"), row.names = FALSE)

# Save summary statistics
mcmc_att_T2_bio_pre_summary <- summary(mcmc_att_bio_T2_pre)
mcmc_meta_T2_bio_pre_summary <- summary(mcmc_meta_bio_T2_pre)
